-- Исправление RPC функции get_user_context
-- Убираем ссылку на несуществующую колонку employee_id
...
BEGIN
  ...
  -- Получаем данные из company_members (без employee_id)
  SELECT 
    cm.role,
    cm.role_id
  ...
  -- Ищем сотрудника напрямую по user_id
  IF v_role_name IS NULL OR v_position IS NULL THEN
    SELECT 
      e.position,
      r.name as role_name,

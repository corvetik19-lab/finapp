...
import { PurchaseLedgerEntry } from "@/lib/accounting/registers/types";
import { getPurchaseLedger } from "@/lib/accounting/registers/service";

interface PurchaseLedgerPageProps {
  initialEntries: PurchaseLedgerEntry[];
  initialYear: number;
...
export function PurchaseLedgerPage({
  ...
  const handleRefresh = async () => {
    setIsLoading(true);
    try {
      const newEntries = await getPurchaseLedger(year, quarter);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">Книга покупок</h1>
          <p className="text-muted-foreground">
            Учёт входящего НДС для налоговых вычетов
          </p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-sm text-muted-foreground">НДС к вычету</div>
      ...
      <Card>
        <CardHeader className="py-3">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            Книга покупок за {QUARTERS.find((q) => q.value === quarter.toString())?.label} {year}
          </CardTitle>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-[120px]">Документ</TableHead>
              ...
              <TableBody>
                {entries.map((entry) => (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell>{formatDate(entry.document_date)}</TableCell>

...
const BG_REMOVAL_MODELS = IMAGE_MODELS.filter(m => m.type === "background-removal");
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <div>
          <h1 className={styles.title}>Удаление фона</h1>
          <p className={styles.subtitle}>Kie.ai • Автоматическое удаление фона</p>
        </div>
      </div>
      ...
      <div className={styles.content}>
        ...
        <div className={styles.modelSection}>
          ...
          <select
            ...
            onChange={(e) => {
              const model = BG_REMOVAL_MODELS.find(m => m.id === e.target.value);
        ...
        >
          {isProcessing ? (
            <>
              ...
              <span>{taskStatus?.state === 'generating' ? 'Обработка...' : 'Ожидание...'}</span>
            </>
          ) : (
            <>
        ...
        <div className={styles.infoBox}>
          <p>
            <strong>Kie.ai Background Removal</strong> автоматически удаляет фон.
            Доступны модели: BRIA, BiRefNet.
          </p>
        </div>
      </div>
    </div>

...
export async function createDefaultAccount() {
  ...
  if (existing && existing.length > 0) return revalidatePath("/finance/transactions");
...
export async function exportTransactionsAction(rawParams: unknown): Promise<Response> {
  ...
  const url = new URL("/finance/transactions/export", "http://localhost");
...
export async function createTransactionFromValues(
  ...
  try {
    ...
    // Автоматически определяем категорию из первого товара, если категория не указана
    let categoryId = parsed.category_id || null;
    if (!categoryId && items && items.length > 0) {
      const firstItemWithCategory = items.find(item => item.category_id);
      if (firstItemWithCategory) {
        categoryId = firstItemWithCategory.category_id || null;
      }
    }
...
export async function updateTransactionFromValues(
  ...
  try {
    ...
    // Обновляем позиции товаров, если они переданы
    if (items !== undefined) {
      const { updateTransactionItems } = await import("@/lib/transactions/transaction-items-service");
      await updateTransactionItems(parsed.id, items);
    }
...
export type DuplicateTxnState = { ok: boolean; error?: string };

export async function duplicateTransactionAction(transactionId: string): Promise<DuplicateTxnState> {
  try {
    const supabase = await createRouteClient();
    const {
      data: { user },
    ...
    if (fetchError || !original) throw fetchError ?? new Error("Транзакция не найдена");

    // Получаем позиции товаров оригинальной транзакции
    const { getTransactionItems } = await import("@/lib/transactions/transaction-items-service");
    const items = await getTransactionItems(transactionId);

    // Создаём новую транзакцию с текущей датой
    ...
    const newTransaction = await createTransactionService({
      ...
      amount_major: original.amount / 100,
    ...
    // Копируем позиции товаров, если они есть
    if (items.length > 0 && newTransaction.id) {
      const itemsToCreate: TransactionItemInput[] = items.map((item) => ({
        name: item.name,
        quantity: item.quantity,
        unit: item.unit,
        price_per_unit: item.price_per_unit,
        total_amount: item.total_amount,
        product_id: ('product_id' in item ? item.product_id : null) as string | null,
        category_id: ('category_id' in item ? item.category_id : null) as string | null,
      }));
